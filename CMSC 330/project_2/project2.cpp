...
// 04/30/2024
// Added handling to clear symbol table after every line to allow variables to be reused.
// Added exception handling to catch uninitialized variable or duplicated initialization of variables.
...
#include "parse.h"
#include "duplicatevariableexception.h"
...
int main() {
    ...
    std::string progStart = "Program Starting";
...
void parseAssignments(stringstream& in) {
    ...
    do {
        ...
        if (symbolTable.lookUp(variable) != -1) {
            throw DuplicateVariableException(variable);
        }
